...
const quizQuestions = ref(
    Array.from({ length: 10 }, () => createEmptyQuestion())
);
...
const resetForm = () => {
    ...
    quizQuestions.value = Array.from({ length: 10 }, () => createEmptyQuestion());
...
const getCourseById = async () => {
    ...
    try {
        ...
        form.value = {
            ...
            currency_arm: data.currency_arm || '',
        ...
        faqList.en = data.faqs?.length
            ? data.faqs.map(f => ({ question_en: f.question_en || '', answer_en: f.answer_en || '' }))
            : [{ question_en: '', answer_en: '' }];

        faqList.ru = data.faqs?.length
            ? data.faqs.map(f => ({ question_ru: f.question_ru || '', answer_ru: f.answer_ru || '' }))
            : [{ question_ru: '', answer_ru: '' }];

        faqList.arm = data.faqs?.length
            ? data.faqs.map(f => ({ question_arm: f.question_arm || '', answer_arm: f.answer_arm || '' }))
            : [{ question_arm: '', answer_arm: '' }];

        highlights.value.en = data.highlights?.length
            ? data.highlights.map(h => h.name_en || '')
            : [''];

        highlights.value.ru = data.highlights?.length
            ? data.highlights.map(h => h.name_ru || '')
            : [''];

        highlights.value.arm = data.highlights?.length
            ? data.highlights.map(h => h.name_arm || '')
            : [''];

        quizQuestions.value = data.questions?.length
        ...
        while (quizQuestions.value.length < 10) {
            quizQuestions.value.push(createEmptyQuestion());
        }

        if (quizQuestions.value.length > 10) {
            quizQuestions.value = quizQuestions.value.slice(0, 10);
        }

        practiceAssignments.value = data.practice_assignments?.length
            ? data.practice_assignments.map(pa => ({
                assignment_text: pa.assignment_text || '',
                correct_answer: pa.correct_answer || '',
            }))
            : [
                { assignment_text: '', correct_answer: '' },
                { assignment_text: '', correct_answer: '' },
        ...
        while (practiceAssignments.value.length < 2) {
            practiceAssignments.value.push({ assignment_text: '', correct_answer: '' });
        }

        showRu.value = !!(form.value.title_ru?.trim() || form.value.description_ru?.trim());
        showArm.value = !!(form.value.title_arm?.trim() || form.value.description_arm?.trim());
    } catch (error) {
...
const removeHighlight = (lang, index) => {
    ...
    if (highlights.value[lang].length === 0) {
        highlights.value[lang].push('');
    }
};
...
const hasError = (field) => {
    return errors.value && errors.value[field] && errors.value[field].length > 0;
};
...
const updateCourse = async () => {
    try {
        ...
        faqList.en.forEach((_, index) => {
            formData.append(`faqs[${index}][question_en]`, faqList.en[index].question_en || '');
            formData.append(`faqs[${index}][answer_en]`, faqList.en[index].answer_en || '');
            formData.append(`faqs[${index}][question_ru]`, faqList.ru[index]?.question_ru || '');
            formData.append(`faqs[${index}][answer_ru]`, faqList.ru[index]?.answer_ru || '');
            formData.append(`faqs[${index}][question_arm]`, faqList.arm[index]?.question_arm || '');
            formData.append(`faqs[${index}][answer_arm]`, faqList.arm[index]?.answer_arm || '');
        });

        highlights.value.en.forEach((_, index) => {
            formData.append(`highlights[${index}][name_en]`, highlights.value.en[index] || '');
            formData.append(`highlights[${index}][name_ru]`, highlights.value.ru[index] || '');
            formData.append(`highlights[${index}][name_arm]`, highlights.value.arm[index] || '');
        });
        ...
        successMessage.value = t('auth.courses.course_updated');
        errors.value = {};

        router.push('/instructor/my-courses?page=1');
        ...
        if (error.response?.data?.errors) {
            ...
            errors.value = { general: [error.message || 'Unknown error'] };
        }
    } finally {
        isLoading.value = false;
    }
        ...
        <div class="container">
            <div class="add-course-main">
                <div class="d-flex justify-content-between align-items-center course-title-block">
                    <h3 class="text-capitalize add-course-section-title"> {{ $t('auth.courses.change_course') }}</h3>
                </div>
                        ...
                        <form class="form">
                            ...
                            <div v-if="showEn">
                                ...
                                <div class="mb-3">
                                    ...
                                    <span v-if="errors.description_en" class="required-field">{{
                                        t('auth.add_course.' + errors.description_en)
                                        }}</span>
                                </div>
                            ...
                            <div v-if="showRu">
                                ...
                                <div class="row mb-3">
                                    <div class="col-md-6">
                                        ...
                                        <input v-model="form.title_ru" type="text" class="form-control"
                                               :placeholder="$t('auth.add_course.course_title_placeholder')">
                                        <span v-if="errors.title_ru" class="required-field">{{
                                            t('auth.add_course.' + errors.title_ru)
                                            }}</span>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label">{{ $t('auth.add_course.course_price') }}</label>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea v-model="form.description_ru" class="form-control" rows="4"
                                              :placeholder="$t('auth.add_course.description_placeholder')"></textarea>
                                    <span v-if="errors.description_ru" class="required-field">{{
                                        t('auth.add_course.' + errors.description_ru)
                                        }}</span>
                                </div>
                            </div>
                            ...
                            <div v-if="showArm">
                                ...
                                <div class="row mb-3">
                                    <div class="col-md-6">
                                        ...
                                        <input v-model="form.title_arm" type="text" class="form-control"
                                               :placeholder="$t('auth.add_course.course_title_placeholder')">
                                        <span v-if="errors.title_arm" class="required-field">{{
                                            t('auth.add_course.' + errors.title_arm)
                                            }}</span>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label">{{ $t('auth.add_course.course_price') }}</label>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="mb-3">
                                        <input v-model="faqList.arm[0].question_arm" type="text"
                                ...
                                <div class="mb-3">
                                    ...
                                    <span v-if="errors.description_arm" class="required-field">{{
                                        t('auth.add_course.' + errors.description_arm)
                                        }}</span>
                                </div>
                            </div>
