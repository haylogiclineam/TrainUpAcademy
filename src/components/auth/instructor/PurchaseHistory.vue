...
const fetchPayments = async (page = 1) => {
    ...
    try {
        const res = await api.get(`/api/instructor/wallet/history?page=${page}`);
        if (res.data.success && res.data.history) {
            payments.value = res.data.history.data || [];
            currentPage.value = res.data.history.current_page || 1;
            totalPages.value = res.data.history.last_page || 1;
        }
    } catch (e) {
                ...
                <div v-else-if="hasPayments" class="purchase-history">
                    <div class="table-wrapper">
                        <table class="purchase-table">
                            ...
                            <tr v-for="(payment, index) in payments" :key="payment.id || index">
                                ...
                                <td>{{ formatAmount(payment.net_amount) }}</td>
                                <td>
                    ...
                    <div class="mobile-wrapper">
                        <div
                            ...
                            <div class="mobile-row">
                                <p class="mobile-label">{{ $t('purchase_history.service_fee') }}</p>
                                <hr>
                                <p class="mobile-label-content">{{ formatAmount(payment.platform_fee) }}</p>
                            </div>
                            <hr>

                            <div class="mobile-row">
                                <p class="mobile-label">{{ $t('purchase_history.net') }}</p>
                                <hr>
                                <p class="mobile-label-content">{{ formatAmount(payment.net_amount) }}</p>
                            </div>
                            <hr>
